<template>
    <index-item v-slot:MainRight :user="userLogin" :isActive="isActive" :notify="notify" :result="result">
        <div class="bg-white h-100 p-3">
            <h5 class="d-inline-block font-weight-bold">Danh sách người dùng</h5>
            <ul class="nav">
                <li class="nav-item ml-auto">
                    <router-link :to="{name: 'UserStore'}" class="btn btn-warning">
            ...
            <div class="mt-4">
                <table class="table table-striped  text-center" id="userList">
                    ...
                    <tbody>
                            ...
                            <tr v-for="(user, index) in users" :key="user.id" class="delete">
                                <td>{{ setStt(index)}}</td>
                                <td>
                                ...
                                <td>
                                    ...
                                    <router-link :to="{name: 'UserEdit', params: {id: user.id}}" class="btn btn-warning"><i class="fa fa-pencil-square" aria-hidden="true"></i></router-link>
                                    <button @click="deleteItem(user.id, index)" class="btn btn-danger"><i class="fa fa-trash" aria-hidden="true"></i></button>
                                </td>
                            </tr>
        ...
        diaLogDelete(id, index) {
            this.$dialog({
                ...
                btns: [{
                        ...
                        color: '#09f',
                        callback: async () => {
                            if(this.userLogin.id == id){
                                this.result = false;
                                this.notify = 'Tài khoản này hiện đang đăng nhập nên không thể xóa'
                                this.isActive = true;
                                setTimeout(() => { this.isActive = false }, 3000);
                            }else{
